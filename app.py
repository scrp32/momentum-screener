...
def check_password():
    ...
    def password_entered():
        # Check against hardcoded password (or st.secrets["password"])
        if st.session_state["password_input"] == "quant123":  # 👈 SET YOUR PASSWORD HERE
            st.session_state["password_correct"] = True
            del st.session_state["password_input"]  # Don't store password in memory
        else:
            st.session_state["password_correct"] = False

    if "password_correct" not in st.session_state:
        # First load: show login prompt
...
if check_password():
    ...
    def fetch_universe_tickers(universe_name):
        urls = {
            ...
            "Nifty Total Market (~750)": "https://archives.nseindia.com/content/indices/ind_niftytotalmarket_list.csv",
        }
    ...
    benchmark_ticker = st.sidebar.selectbox(
        ...
        ["^NSEI", "^BSESN"],
        format_func=lambda x: "Nifty 50 (^NSEI)" if x == "^NSEI" else "Sensex (^BSESN)",
    )
    ...
    max_dma_extension = st.sidebar.slider(
        ...
        30,
        step=5,
        help="Filters out stocks trading too far above their 200-day moving average.",
    )
    ...
    def compute_quant_momentum(tickers, benchmark):
        ...
        raw_data = yf.download(all_tickers, period="1y", interval="1d", progress=False)
    ...
    with st.spinner(f"Fetching data for {len(tickers_list)} stocks in {universe_type}..."):
